            ...
            <div class="panel-heading">
                ...
                <div class="panel-heading-controls">
                    <button ng-click="toggle = !toggle" class="btn-panel">Add
						New Task</button>
                    <button class="btn-panel" confirmed-click="archiveTasks()" ng-confirm-click="Would you like to archive completed tasks?">Clear
						completed tasks</button>
                </div>
            </div>
            <div class="panel-body">
                <div class="task" ng-repeat="task in tasks">
                    <span ng-if="task.taskpriority=='HIGH'" class="priority priority-red"> {{task.taskpriority}} </span> <span ng-if="task.taskpriority=='MEDIUM'" class="priority priority-yellow"> {{task.taskpriority}} </span> <span ng-if="task.taskpriority=='LOW'" class="priority priority-green">
						{{task.taskpriority}} </span>
                    <div class="action-checkbox">
                        <input id="{{task._links.self.href}}" type="checkbox" value="{{task._links.self.href}}" ng-checked="selection.indexOf(task._links.self.href) > -1" ng-click="toggleSelection(task._links.self.href)" /> <label for="{{task._links.self.href}}"></label>
                    </div>
            ...
            <div class="panel-body">
                <div class="task">
                    <table class="add-task">
                        ...
                        <tr>
                            <td>Task Status:</td>
                            <td><select ng-model="taskstatus" ng-options="status as status for status in statuses">
									<option value="">-- Select --</option>
							</select></td>
                        </tr>
                        <tr>
                            <td>Task Priority:</td>
                            <td><select ng-model="taskPriority" ng-options="priority as priority for priority in priorities">
									<option value="">-- Select --</option>
							</select></td>
                        </tr>
                        <tr>
